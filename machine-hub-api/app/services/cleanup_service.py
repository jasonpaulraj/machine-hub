...
class CleanupService:
    ...
    async def start(self):
        ...
        logger.info(f"📊 Keeping latest {self.max_records_per_machine} records per machine")
        if self.cleanup_old_records_days:
            logger.info(f"🗓️ Also removing records older than {self.cleanup_old_records_days} days")
    ...
    async def _run_cleanup(self):
        ...
        try:
            ...
            if deleted_by_count > 0:
                logger.info(f"📊 Deleted {deleted_by_count} records to maintain {self.max_records_per_machine} records per machine")
            ...
            if self.cleanup_old_records_days:
                ...
                if deleted_by_age > 0:
                    logger.info(f"🗓️ Deleted {deleted_by_age} records older than {self.cleanup_old_records_days} days")
            
            if total_deleted == 0:
                logger.debug("✅ No cleanup needed - database is within limits")
            else:
                logger.info(f"✅ Cleanup completed - total {total_deleted} records deleted")
    ...
    async def run_cleanup_now(self) -> dict:
        ...
        try:
            ...
            results["total_deleted"] = deleted_by_count + results["deleted_by_age"]
            
            logger.info(f"✅ Immediate cleanup completed - {results['total_deleted']} records deleted")
            return results
